export default (route) => (req, res, next) => {
   ...
   switch (route.method.toLowerCase()) {
      case 'get': check = global.validator.get(req, allParams); break
      case 'post': case 'put': case 'patch': check = global.validator.post(req, allParams); break
      case 'delete': check = global.validator.delete(req, allParams); break
   ...
   if (apikey !== process.env.SITE_API_KEY)
      return res.status(403).json({ creator: global.creator, status: false, msg: 'Invalid apikey' })
