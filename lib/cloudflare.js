...
export class CloudflareIPBlocker {
   ...
   async blockIP(ip, note = 'Blocked due to spam') {
      try {
         ...
         if (!response.data.success) console.error('Failed to block IP:', response.data.errors)
         else console.log(`Blocked IP: ${ip}`)
         return response.data
      } catch (error) {
         console.error(error.response ? 'API error:' : 'Request error:', error.response?.data ?? error.message)
      }
   }
